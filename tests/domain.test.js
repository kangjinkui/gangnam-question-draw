...
describe('질문 검증',()=>{it('10~100자만 허용한다',()=>{expect(validateQuestion('짧다')).toBeTruthy();expect(validateQuestion('충분히 긴 올바른 질문입니다.')).toBe('');expect(validateQuestion('가'.repeat(100))).toBe('');expect(validateQuestion('가'.repeat(101))).toBeTruthy()})});
